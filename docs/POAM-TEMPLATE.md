...
|---|---|---|---|---|---|---|
| POAM-2026-001 | SC-28 | S3 bucket created outside this repo's templates found without default encryption | Checkov CI scan | Moderate | 90 days from detection | In Progress |

## Tying this repo's CI into your POA&M process

Every Checkov/Trivy finding that gets a `checkov: skip:` metadata block in
a template is a *documented, accepted* design decision — those are not
POA&M candidates, they're already-resolved risk acceptances with a stated
reason.
...
your official POA&M workbook — not as template comments.
