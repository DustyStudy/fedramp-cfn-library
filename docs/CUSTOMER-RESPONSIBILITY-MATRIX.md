...
What this file does instead: maps the controls this repo's templates touch
to (a) AWS's general responsibility model for IaaS, (b) what this repo's
templates automate on the customer side, and (c) what remains a manual
...
| RA-5 (Vulnerability Scanning) | AWS infrastructure-level scanning | GuardDuty, ECR scan-on-push (`modules/guardduty-org`, `modules/ecr-hardened`) | Authenticated OS/application vulnerability detection on the CR26 VDR cadence (monthly for Rev5) — see `CONTINUOUS-MONITORING.md` |
...
| SC-12 / SC-13 / SC-28 (Cryptography & Encryption at Rest) | Physical key storage hardware (HSMs backing KMS) | KMS CMKs with rotation across nearly every template | Cryptographic module validation for anything outside AWS-managed KMS (e.g., in-application crypto) |
...
(PS, AT, PM) — no CloudFormation template addresses them because none
should. See `COVERAGE-GAPS.md` for the full list of what's out of scope
and why.
