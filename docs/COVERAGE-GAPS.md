...
**this repo does not and cannot provide** — so nobody mistakes "we
deployed this repo" for "we're FedRAMP ready."
...
| **AT** — Awareness and Training | Security awareness training program and records — a training curriculum and LMS, not templates. |
...
## Partial coverage — templates exist, but the control needs more than infra
...
- **AC-2 (Account Management):** The password policy and MFA enforcement
  ...
  not something CloudFormation can enforce on its own.
...
- `fips-vpc-endpoints` only has genuine FIPS-suffixed endpoints for `kms`,
  `ec2`, and `sts` — see that template's comments for why the rest aren't
  actually FIPS-specific endpoints.
...
- `moderate/` and `high/` copies of `account-baseline`, `org-governance`,
  and `org-scp-boundary` duplicate their `modules/` counterpart's logic
  inline rather than referencing it as a nested stack — see
  `moderate/README.md` for why that matters if you change the shared
  logic later.
